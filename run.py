...
def download_zip():
    ...
    folder = resource_path(os.path.join("generated", nic))
    downloads_dir = resource_path("downloads")
    zip_base = os.path.join(downloads_dir, f"{nic}-{time_stamp}")
    ...
    return send_from_directory(downloads_dir, os.path.basename(zip_path), as_attachment=True)
